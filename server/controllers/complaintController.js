...
// Staff: Submit update for a complaint (with photo, remarks, and status)
const staffUpdateComplaint = async (req, res) => {
    try {
        ...
        // Only allow the staff member this complaint is assigned to
        const staffId = req.user.id || req.user._id;
        if (!complaint.assignedTo || complaint.assignedTo.toString() !== staffId) {
            return res.status(403).json({ message: 'You are not assigned to this complaint.' });
        }

        // Push the progress update (photo/remarks log)
        if (remarks || photoUrl) {
            complaint.staffUpdates.push({ photoUrl, remarks, updatedAt: new Date() });
        }

        // Validate and persist status if provided
        const allowedStatuses = ['pending', 'in-progress', 'resolved'];
        const statusChangedToResolved = status && status !== complaint.status && status === 'resolved';
        ...
        // Notify the student if staff marked it resolved
        if (statusChangedToResolved && complaint.raisedBy && complaint.raisedBy.email) {
            try {
                await sendResolutionEmail(
                    complaint.raisedBy.email,
                    'Your complaint has been resolved',
                    `Hello ${complaint.raisedBy.name},\n\nYour complaint titled "${complaint.title}" has been marked resolved.\n\nThank you.`
                );
            } catch (mailError) {
                console.error('Error sending resolution email:', mailError);
            }
